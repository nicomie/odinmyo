...
createSwapchain :: proc(ctx: ^Context) {
	...
	queueFamilyIndices := [len(QueueFamily)]u32 {
		cast(u32)queueIndices[.Graphics],
		cast(u32)queueIndices[.Present],
	}
	...
	for i in 0 ..< swapchain.imageCount {
		if vk.CreateSemaphore(device, &semaphoreInfo, nil, &ctx.renderFinishedSemaphores[i]) !=
		   .SUCCESS {
