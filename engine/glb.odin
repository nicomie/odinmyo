...
processMeshNode :: proc(
	...
	append(&rm.meshObjects, MeshObject{mesh_index, world_transform})

	fmt.printf(
		"Created MeshObject %d with transform for node '%s'\n",
		mesh_index,
		node.name != nil ? string(node.name) : "unnamed",
	)
}
...
processMaterials :: proc(rm: ^ResourceManager, data: ^cgltf.data, ctx: ^Context) -> []Material {
	...
	for i in 0 ..< len(data.materials) {
		...
		m.name =
			strings.clone_from_cstring(gm.name) if gm.name != nil else fmt.tprintf("material_%d", i)
...
createTextureImageFromPixels :: proc(
	ctx: ^Context,
	texture: ^Texture,
	pixels: rawptr,
	w, h: i32,
