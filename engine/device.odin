...
createLogicalDevice :: proc(ctx: ^Context) {
	...
	if vk.CreateDevice(ctx.vulkan.physicalDevice, &createInfo, nil, &ctx.vulkan.device) !=
	   .SUCCESS {
	...
	vk.GetDeviceQueue(
		ctx.vulkan.device,
		u32(ctx.vulkan.queueIndices[.Graphics]),
		0,
		&ctx.vulkan.graphicsQueue,
	)
	vk.GetDeviceQueue(
		ctx.vulkan.device,
		u32(ctx.vulkan.queueIndices[.Present]),
		0,
		&ctx.vulkan.presentQueue,
	)
