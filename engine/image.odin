...
createImageViews :: proc(ctx: ^Context) {
	...
	for _, i in swapchain.images {
		swapchain.attachments.views[i] = createImageView(
			ctx,
			swapchain.images[i],
			swapchain.format,
			{.COLOR},
...
createTextureImageView :: proc(ctx: ^Context) {
	...
	textures[0].view = createImageView(
		ctx,
		textures[0].handle.texture,
		.R8G8B8A8_SRGB,
		{.COLOR},
		textures[0].mips,
		"texture",
...
createTextureImage :: proc(ctx: ^Context, texture: ^Texture, path: string, textureIndex: int) {
	device := ctx.vulkan.device
...
createFontTextureImage :: proc(
	...
	if len(atlas_data) != int(imageSize) {
		fmt.eprintf(
			"ERROR: Font atlas data size mismatch. Expected %d, got %d\n",
			imageSize,
			len(atlas_data),
		)
		return
